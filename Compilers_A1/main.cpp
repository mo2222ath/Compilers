...
enum TokenType
{
    ...
    PLUS, MINUS, TIMES, DIVIDE, POWER, ABS_DIF , // Add Absolute difference TokenType
    SEMI_COLON,
...
const char* TokenTypeStr[]=
{
    ...
    "Plus", "Minus", "Times", "Divide", "Power", "AbsDif", // Add Absolute difference TokenTypeStr
    "SemiColon",
...
const Token symbolic_tokens[]=
{
    ...
    Token(DIVIDE, "/"),
    Token(POWER, "^"),
    Token(ABS_DIF,"&"),   // Add Absolute difference symbolic_tokens
    Token(SEMI_COLON, ";"),
    Token(LEFT_PAREN, "("),
    Token(RIGHT_PAREN, ")"),
    Token(LEFT_BRACE, "{"),
...
// mathexpr -> absdif { (+|-) absdif }    left associative ( edit mathexpr go to absdif instead of term to be higher precedence than +,- and lower precedence than *,/  )
// absdif -> term { & term }      left associative  ( add absdif{&} )
...
// absdif -> term { & term }      left associative  // Absolute difference function
TreeNode* AbsDif(CompilerInfo* pci, ParseInfo* ppi)
{
    pci->debug_file.Out("Start AbsDif"); // write to file the debug that start with this function

    TreeNode* tree=Term(pci, ppi); // make tree with term function to take children from high precedence function
    while(ppi->next_token.type==ABS_DIF)  // this is loop if there is multi absolute difference function after check if the token type is ABS_DIF
    {
        TreeNode* new_tree=new TreeNode; // make tree for take info about the absolute difference function
        new_tree->node_kind=OPER_NODE; //
        new_tree->oper=ppi->next_token.type; // assign the token type to the tree opration
        new_tree->line_num=pci->in_file.cur_line_num; // assign the number of line in code to the number in tree

        new_tree->child[0]=tree; // make it as child for tree
        Match(pci, ppi, ppi->next_token.type); // match function is check if the next token is the expected token
        new_tree->child[1]=Term(pci, ppi); // make another term for the next token

        tree=new_tree;
    }
    pci->debug_file.Out("End AbsDif");
    return tree;
}

// mathexpr -> absdif { (+|-) absdif }    left associative 'edited' and make it call the absdif function instead of term function
TreeNode* MathExpr(CompilerInfo* pci, ParseInfo* ppi)
{
    pci->debug_file.Out("Start MathExpr");

    TreeNode* tree=AbsDif(pci, ppi);
    ...
    while(ppi->next_token.type==PLUS || ppi->next_token.type==MINUS)
    {
        ...
        new_tree->child[0]=tree;
        Match(pci, ppi, ppi->next_token.type);
        new_tree->child[1]=AbsDif(pci, ppi);
...
int Evaluate(TreeNode* node, SymbolTable* symbol_table, int* variables)
{
    ...
    if(node->oper==ABS_DIF) return AbsDif(a,b);
    throw 0;
    return 0;
}
