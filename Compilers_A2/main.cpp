...
x := 2;     {assign to int}
y := 3;     {assign to int}
a := 2.5;    {assign to double or real}
b := 1.0;    {assign  to double or real}
t := 1;    {assign to boolean}
f := 0;    {assign to boolean}
z := 4;    { error it's not declared }

x := y + 2;  {the result for x is 5}
y := y * 2;  {the result for y is 6}

x := y + a;  {error int with real}

x := t + 2;  {error bool is not allowed on arthematic operation}

if t then
  x := 1;

a := b + 2.4;   {the answer will be double or real couse all double}
a := b + 1;     {error real with int!}

t := 5;         {error assign boolean variable with integer}
...
[Var=a][Mem=2][Line=5]
[Var=b][Mem=3][Line=6]
[Var=f][Mem=5][Line=8][Line=15]
[Var=t][Mem=4][Line=7][Line=14]
[Var=x][Mem=0][Line=3][Line=11][Line=17]
[Var=y][Mem=1][Line=4][Line=12]
---------------------------------
Syntax Tree:
[Declare][x][Integer]
[Declare][y][Integer]
...
const char* TokenTypeStr[]=
            {
                ...
                "ID", "Num",
                "EndFile", "Error","Declare"   // add declare token
            };
...
// return if it's data type or not
inline bool IsDataType(char* s)
{
    if (StartsWith(s, "int")) return true;
    else if (StartsWith(s, "real")) return true;
    else if (StartsWith(s, "bool")) return true;
    else return false;
}
...
void GetNextToken(CompilerInfo* pci, Token* ptoken)
{
    ...
    // check if the token is data type
    else if(IsDataType(s)){
        test.Out("IsDataType(s)");
        test.Out(s);
        int j = 1;
        ...
        ptoken->type = DECLARE;
        Copy(ptoken->str, s, j);
        test.Out("ptoken->str in dataType");
        test.Out(ptoken->str);
    }
...
// declareStmt -> declare identifier     \\ add declare to EBNF
...
const char* ExprDataTypeStr[]=
            {
                "Void", "Integer", "Boolean" , "Real" // add data types as string
            };

// return ExprDataType depend for the token string
ExprDataType getDataType(char type[5]){
    if(StartsWith(type,"real")) return REAL;
    else if(StartsWith(type,"bool")) return BOOLEAN;
    else return INTEGER;
}
...
struct TreeNode
{
    ...
    TokenType oper; 
    int intnum;
    //double realnum; bool boolnum;
    char* id;}; // defined for expression/int,real and bool/identifier only
...
void Match(CompilerInfo* pci, ParseInfo* ppi, TokenType expected_token_type)
{
    ...
    char tempType[5];
    if(ppi->next_token.type != DECLARE) Copy(tempType, ppi->next_token.dataType);
    GetNextToken(pci, &ppi->next_token);
    Copy(ppi->next_token.dataType,tempType);
...
TreeNode* NewExpr(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    if(ppi->next_token.type==NUM)
    {
        ...
        // if(StartsWith(ppi->next_token.dataType,"int")){
            tree->intnum=0; while(*num_str) tree->intnum=tree->intnum*10+((*num_str++)-'0');
        // }
        // else if(StartsWith(ppi->next_token.dataType,"real")){
        //     tree->realnum=0; while(*num_str) tree->realnum=tree->realnum*10+((*num_str++)-'0');
        // }
        // else if(StartsWith(ppi->next_token.dataType,"bool")){
        //     tree->boolnum=0; while(*num_str) tree->boolnum=tree->boolnum*10+((*num_str++)-'0');
        // }
        tree->line_num=pci->in_file.cur_line_num;
        tree->expr_data_type = getDataType(ppi->next_token.dataType);
...
TreeNode* Factor(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    if(ppi->next_token.type==POWER)
    {
        ...
        new_tree->expr_data_type = getDataType(ppi->next_token.dataType);
...
// declarestmt -> declare identifier
TreeNode* DeclareStmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    tree->line_num=pci->in_file.cur_line_num;
    // set the data type to the tree
    // tree->expr_data_type = getDataType(ppi->next_token.dataType);

    char tempType[5];
    // to find the data type and set for tree and copy it to the another tokens
    if(ppi->next_token.str[0] == 'r'){
        tree->expr_data_type = REAL; 
        Copy(ppi->next_token.dataType , "real");
        Copy(tempType,"real");
    } 
    else if(ppi->next_token.str[0] == 'b')
    {
        tree->expr_data_type = BOOLEAN;
        Copy(ppi->next_token.dataType , "bool");
        Copy(tempType,"bool");
    }
    else {
        tree->expr_data_type = INTEGER; 
        Copy(ppi->next_token.dataType , "int");
        Copy(tempType,"int");
    }
    // test.Out("ppi->next_token.str in declare function");
    // test.Out(ppi->next_token.str);

    Match(pci, ppi, DECLARE);
    if(ppi->next_token.type==ID) AllocateAndCopy(&tree->id, ppi->next_token.str);
    Copy(ppi->next_token.dataType,tempType);
    Match(pci, ppi, ID);
...
TreeNode* Stmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    else if(ppi->next_token.type==DECLARE) tree=DeclareStmt(pci, ppi);
    else throw 0;
...
void PrintTree(TreeNode* node, int sh=0)
{
    ...
    if(node->node_kind==OPER_NODE) printf("[%s]", TokenTypeStr[node->oper]);
        ...
        // if(node->expr_data_type == INTEGER)
            printf("[%d]", node->intnum);
        // else if(node->expr_data_type == REAL)
            // printf("[%d]", node->realnum);
        // else if(node->expr_data_type == BOOLEAN)
            // printf("[%d]", node->boolnum);
    }
    else if(node->node_kind==ID_NODE || node->node_kind==READ_NODE || node->node_kind==ASSIGN_NODE || node->node_kind== DECLARE_NODE) printf("[%s]", node->id);
...
struct SymbolTable
{
    ...
    void Insert(const char* name, int line_num)
    {
        ...
        // test.Out(type +" <---- Type in Insert");
        // test.Out(" ---- name in Insert ");
        // test.Out(vi->name);
        // if(type != VOID) vi->type = type;
...
void Analyze(TreeNode* node, SymbolTable* symbol_table)
{
    ...
    // if( node->node_kind==DECLARE_NODE) symbol_table->Insert(node->id, node->line_num , node->expr_data_type);

    if( node->node_kind==DECLARE_NODE ||node->node_kind==ID_NODE || node->node_kind==READ_NODE || node->node_kind==ASSIGN_NODE)
    ...
    if(node->node_kind==OPER_NODE)
    {
        ...
        // check if the two data type the same or not
        else if (node->oper== PLUS || node->oper==MINUS || node->oper== TIMES || node->oper==DIVIDE){
            if(node->child[0]->expr_data_type == REAL && node->child[1]->expr_data_type == REAL) node->expr_data_type=REAL;
            else if(node->child[0]->expr_data_type == INTEGER && node->child[1]->expr_data_type == INTEGER) node->expr_data_type=INTEGER;
            else {throw 0;}
        }
    }
    ...
    //     // node->expr_data_type=varInfo->type;
    // }
